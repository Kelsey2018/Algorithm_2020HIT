...
#         pass
#
#     def double_direction(self):
#         pass
#
#
# if __name__ == '__main__':
#     test_a = graph(width=10,height=10)
#     test_b = graph(width=10,height=10)
#     a_star = A_star(graph=test_a)
#     a_star.single_direction()
#     a_star.double_direction()
...
for x in range(num_cells):
    for y in range(num_cells):
        cells[(x, y)] = {'state': None,  # None, Wall, Goal, Start Are the possible states. None is walkable
                         ...
                         'h_score': None,
                         # The heuristic score, We use straight-line distance: sqrt((x1-x0)^2 + (y1-y0)^2)
...
def diagonals(current):
    ...
    return [x for x in directions if
            onBoard(x) and cells[x]['state'] != 'Wall' and not x in closed_list and not blocked_diagnol(current, x)]
...
while 1:
    for event in pygame.event.get():
        ...
        if (x_index, y_index) in cells:
            ...
            # Place Wall
            elif shift and left_click and cells[(x_index, y_index)]['state'] == None and last_wall != (
            x_index, y_index):
            ...
            # Remove Wall
            elif shift and left_click and cells[(x_index, y_index)]['state'] == 'Wall' and last_wall != (
            x_index, y_index):
                cells[(x_index, y_index)]['state'] = None
                r = pygame.Rect(left, top, cell_size - 2, cell_size - 2)
                pygame.draw.rect(board, gray, r, 0)
            ...
            # Soft Reset Board (keep start, goal and walls)
                ...
                i = 0
                for cell in cells:
                    if cells[cell]['state'] != 'Wall' and cells[cell]['state'] != 'Start' and cells[cell][
                        'state'] != 'Goal':
                        i += 1
                        for x in cells[cell]:
                            cells[cell][x] = None
